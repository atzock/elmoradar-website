	...
	<div>
		...
		<section class="relative pt-8 pb-12 border-b border-signal-500/10 overflow-hidden">
			<div class="relative max-w-2xl">
				<h1 class="font-display text-4xl sm:text-5xl font-bold tracking-tight mb-5">
					Hardware & Settings
				</h1>
		...
		<!-- ── SUBCHAPTER NAV ───────────────────────────────────── -->
		<nav class="py-5 border-b border-signal-500/10 flex flex-wrap gap-2">
			{#each [
				...
				<a
					href="#{item.id}"
					class="hud-panel-sm px-3 py-1.5 text-xs font-mono text-signal-500/60 bg-white/3 border border-signal-500/15 hover:bg-signal-600/10 hover:text-signal-400 hover:border-signal-500/30 transition-all"
				>
					{item.label}
				</a>
			{/each}
		...
		<!-- ── RECHNER ───────────────────────────────────────────── -->
		<section id="rechner" class="reveal py-12 border-b border-signal-500/10" use:reveal>
			<h2 class="text-sm font-display font-semibold text-white/60 mb-1">Rechner</h2>
			<p class="text-xs font-mono text-signal-500/50 mb-8">Der Kern von allem.</p>
			...
			<div class="grid sm:grid-cols-2 gap-4 mb-6">
				{#each featured as item}
					...
					>
						...
						<div class="hud-panel overflow-hidden border bg-linear-to-br {item.gradient} p-6">
							<div class="flex items-start gap-4">
								<div class="hud-panel-sm p-3.5 border {item.iconColor} shrink-0">
									<item.icon size={22} />
								</div>
								<div class="flex-1 min-w-0">
									<div class="flex items-center justify-between gap-2 mb-1">
										<span class="text-[10px] font-mono text-signal-500/60 uppercase tracking-widest">{item.label}</span>
										<ArrowUpRight size={13} class="text-white/15 group-hover:text-white/40 transition-colors shrink-0" />
									</div>
									<div class="font-display text-lg font-bold text-white leading-tight mb-1">{item.value}</div>
			...
			<div class="hud-panel border border-signal-500/10 overflow-hidden divide-y divide-signal-500/8">
				{#each specs as s}
					{#if s.url}
						<a
							href={s.url}
							target="_blank"
							rel="noopener noreferrer"
							class="flex items-center gap-4 px-5 py-3.5 hover:bg-signal-500/5 transition-colors group"
						>
							<s.icon size={14} class="{s.iconColor} shrink-0 opacity-60 group-hover:opacity-100 transition-opacity" />
							<span class="text-xs font-mono text-white/35 w-20 shrink-0">{s.label}</span>
							<span class="text-sm text-white/75 flex-1">{s.value}</span>
							<ExternalLink size={12} class="text-signal-500/30 group-hover:text-signal-400 transition-colors shrink-0" />
						</a>
					{:else}
			...
			<div class="grid grid-cols-2 gap-3 mt-5">
				<div class="hud-panel overflow-hidden border border-signal-500/10 aspect-video bg-white/3">
		...
		<!-- ── SCHREIBTISCH ──────────────────────────────────────── -->
		<section id="schreibtisch" class="reveal py-12 border-b border-signal-500/10" use:reveal>
			<h2 class="text-sm font-display font-semibold text-white/60 mb-1">Schreibtisch</h2>
			<p class="text-xs font-mono text-signal-500/50 mb-8">Was sonst noch so auf dem Tisch und drumherum steht.</p>

			<div class="grid sm:grid-cols-3 gap-4">
				{#each deskGroups as group}
					<div class="hud-panel border border-signal-500/10 overflow-hidden">
						<div class="px-4 py-3 border-b border-signal-500/10 bg-white/2">
							<span class="text-[11px] font-mono text-signal-500/60 uppercase tracking-widest">{group.heading}</span>
						</div>
						<div class="divide-y divide-signal-500/8">
							{#each group.items as item}
								{#if item.url}
									<a
										...
										target="_blank"
										rel="noopener noreferrer"
										class="flex items-start gap-3 px-4 py-3 hover:bg-signal-500/5 transition-colors group"
									>
										<item.icon size={13} class="text-white/30 shrink-0 mt-0.5 group-hover:text-signal-400 transition-colors" />
										<div class="min-w-0 flex-1">
											<div class="text-[10px] text-white/25 mb-0.5">{item.label}</div>
											<div class="text-sm text-white/75 leading-snug">{item.value}</div>
										</div>
										<ExternalLink size={11} class="text-white/10 group-hover:text-signal-400/60 transition-colors shrink-0 mt-0.5" />
									</a>
								{:else}
		...
		<!-- ── PEIKER MIKROFON ───────────────────────────────────── -->
		<section id="peiker" class="reveal py-12 border-b border-signal-500/10" use:reveal>
			<div class="flex items-center gap-2 mb-1">
				<Mic size={13} class="text-signal-400/70" />
				<h2 class="text-sm font-display font-semibold text-white/60">Peiker-Mikrofon</h2>
				<span class="hud-panel-sm ml-1 text-[10px] font-mono uppercase tracking-widest border px-2 py-0.5 bg-signal-500/10 text-signal-300 border-signal-500/25"
					>Häufig gefragt</span
				>
			</div>
			<p class="text-xs font-mono text-signal-500/50 mb-8">Das Mikrofon, das am häufigsten im Chat auftaucht.</p>

			<div class="glow-card hud-panel relative" use:glow>
				<span class="glow-border" aria-hidden="true"></span>
				<div
					class="hud-panel border border-signal-500/20 bg-linear-to-br from-signal-500/10 via-signal-500/5 to-transparent overflow-hidden"
				>
					<div class="flex flex-col lg:flex-row">
						...
						<div class="flex-1 p-6 sm:p-8">
							<div class="flex items-start gap-3 mb-5">
								<div class="hud-panel-sm p-3 border border-signal-500/25 bg-signal-500/15 shrink-0">
									<Mic size={20} class="text-signal-300" />
								</div>
								<div>
									<h3 class="font-display text-lg font-bold text-white leading-tight">Peiker ATC-Mikrofon</h3>
							...
							<div class="hud-panel-sm flex items-start gap-2 px-4 py-3 bg-white/4 border border-signal-500/10">
						...
						<div class="lg:w-72 xl:w-80 shrink-0 p-4 lg:p-6">
							<div class="flex gap-3 h-full">
								<div class="hud-panel-sm flex-1 overflow-hidden border border-signal-500/10 min-h-44 lg:min-h-0">
									<img
										src={peiker1}
		...
		<!-- ── FLUGSIMULATION ────────────────────────────────────── -->
		<section id="flugsimulation" class="reveal py-12 border-b border-signal-500/10" use:reveal>
			<h2 class="text-sm font-display font-semibold text-white/60 mb-1">Flugsimulation</h2>
			<p class="text-xs font-mono text-signal-500/50 mb-8">Die Hardware zwischen mir und dem Himmel.</p>
			...
			<div class="grid sm:grid-cols-3 gap-5">
				{#each flightsim as item}
					...
					>
						<span class="glow-border" aria-hidden="true"></span>
						<div class="hud-panel overflow-hidden border bg-linear-to-br {item.gradient} p-6 h-full">
							<div class="flex items-start justify-between mb-5">
								<div class="hud-panel-sm p-3 border {item.iconColor}">
									<item.icon size={20} />
								</div>
								<span class="text-[10px] font-mono uppercase tracking-widest border rounded-md px-2 py-1 {item.badge}">
									{item.label}
								</span>
							</div>
							<div class="font-display text-base font-semibold text-white mb-2 leading-snug">{item.value}</div>
		...
		<!-- ── MSFS SETTINGS ─────────────────────────────────────── -->
		<section id="msfs" class="reveal py-12" use:reveal>
			<div class="flex items-start justify-between gap-4 mb-8">
				<div>
					<div class="flex items-center gap-2 mb-1">
						<Zap size={13} class="text-signal-400/60" />
						<h2 class="text-sm font-display font-semibold text-white/60">MSFS Settings</h2>
					</div>
					<p class="text-xs font-mono text-signal-500/50 max-w-md leading-relaxed">
			...
			<div class="grid gap-4 sm:grid-cols-2">
				{#each msfsSettings as section}
					<div class="hud-panel border border-signal-500/10 bg-white/2 overflow-hidden border-t-2 {section.accent}">
						<div class="px-5 pt-5 pb-4">
							<div class="flex items-center gap-2 mb-4">
								<Settings size={12} class="text-signal-500/40" />
								<h3 class="text-[11px] font-mono text-signal-500/60 uppercase tracking-widest">{section.title}</h3>
							</div>
							{#each section.items as item}
								<div class="flex justify-between items-center py-2.5 border-b border-signal-500/8 last:border-b-0 text-sm gap-3">
									<span class="text-white/45">{item.label}</span>
									<span class="text-[11px] font-mono font-semibold px-2 py-0.5 rounded-md border shrink-0 {valueColor(item.value)}">
										{item.value}
									</span>
								</div>
							{/each}
						...
						{#if section.note}
							<div class="px-5 py-3 bg-white/2 border-t border-signal-500/8 flex items-start gap-2">
								<Info size={12} class="text-signal-500/30 shrink-0 mt-0.5" />
